...
def fix_product_data():
    with app.app_context():
        ...
        if product:
            ...
            # 检查是否有剩余次数和到期日期
            # 注意：使用正确的字段名remaining_count
            if product['remaining_count'] is None or product['remaining_count'] == '':
                # 从原始产品获取信息
                ...
                if original_product:
                    # 修复剩余次数
                    remaining = original_product.get('total_amount', 30)
                    ...
                    # 更新产品信息 - 使用正确的字段名remaining_count
                    db.execute(
                        'UPDATE client_product SET remaining_count = ?, expiry_date = ? WHERE id = ?',
                ...
                print(f"产品剩余次数正常: {product['remaining_count']}，到期日期: {product['expiry_date']}")
                ...
                if product['expiry_date'] is None:
                    ...
                    db.execute(
                        'UPDATE client_product SET expiry_date = ? WHERE id = ?',
                        (expiry_str, 13)
                    )
                    db.commit()
                    print("到期日期已更新")
