...
def fix_statistics_enhanced():
    """修复并增强统计报表功能，支持显示产品名称和合并操作人员统计"""
    ...
    # 创建备份
    with open('app_simple.py.stats_enhanced_bak', 'w', encoding='utf-8') as f:
        f.write(content)
    ...
    if stats_match:
        print("找到admin_statistics函数，准备增强...")
...
def admin_statistics():
    ...
    try:
        # 首先尝试获取详细的按产品统计结果
        operator_product_usage = db.execute(
            'SELECT o.id, o.name as operator_name, o.position, p.name as product_name, '
            ...
            f'WHERE 1=1 {usage_filter} '
            'GROUP BY o.id, p.id '
            'ORDER BY o.name, total_used DESC',
            usage_params
        ).fetchall()
        ...
        # 如果查询出错，使用空列表
        operator_product_usage = []
        operator_usage = []
        print(f"操作人员使用统计查询出错: {e}")
    
    # 无操作人员的使用记录统计（按记录人统计）- 增强版，包含产品信息
    try:
        # 首先尝试获取详细的按产品统计结果
        user_product_usage = db.execute(
            'SELECT u.id, u.username, u.role, p.name as product_name, '
            'COUNT(cpu.id) as usage_count, SUM(cpu.amount_used) as total_used, '
            'COUNT(DISTINCT cp.client_id) as client_count '
            'FROM client_product_usage cpu '
            ...
            'GROUP BY u.id, p.id '
            'ORDER BY u.username, total_used DESC',
            usage_params
        ).fetchall()
    ...
    return render_template(
        ...
        operator_product_usage=operator_product_usage,
        no_operator_usage=no_operator_usage,
        user_product_usage=user_product_usage,
        ...
        print("已增强admin_statistics函数，添加产品关联和合并统计")
    else:
    ...
    print("已完成对统计功能的增强，请重启应用以应用更改")

if __name__ == "__main__":
    fix_statistics_enhanced() 